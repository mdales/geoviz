...
let render_to_primitives (_ft : float) (s : Screen.t)
	...
	List.filter_map
		(fun (e, c) ->
			...
			match e with
			| Point e -> (
					if e.z > 0. then None else
					Some (Primitives.Pixel (project s e, if col = 0 then 1 else 3))
					)
			| Line (a, b) ->
					Some (Primitives.Line
						(project s a, project s b, col / if a.z < 0. then 1 else 3))
			| Triangle (a, b, c) ->
					Some (Primitives.FilledTriangle (project s a, project s b, project s c, col))
			| Polygon vl ->
					let rep = get_represent_vec e in
					Some (Primitives.FilledPolygon
...
let tick t s prev (inputs : Base.input_state) =
	...
	let fb, new_state = match !state, event_state, input_state with
