...
let load_data_from_geotiff filename outputQ =
  ...
  let width = Tiff.Ifd.width ifd
  and height = Tiff.Ifd.height ifd in
  ...
  for y = 0 to (height / scale) do
    let window = Tiff.{ xoff = 0; yoff = y * scale; xsize = width; ysize = 1 } in
...
let rec worker inputQ outputQ =
  ...
  | Task filename -> (
    (try
      load_data_from_file filename outputQ
    with
